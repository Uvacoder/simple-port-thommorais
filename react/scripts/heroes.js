...
// export const http = axios.create({ baseURL: 'https://gateway.marvel.com/v1/public/' })
// export const publicKey = '47c9473ad09457b33870db0fcb78566c'
// export const privateKey = 'b4b0461743fa587b955871b49e56a5ede9e6be89'

var key = "47c9473ad09457b33870db0fcb78566c";

function makeApiCall(urlPart){
	
	urlPart = urlPart||""
	const url = "http://gateway.marvel.com/v1/public/" + urlPart + (urlPart.indexOf("?") >= 0 ? "&" : "?") + "apikey="+key;
		...
		return (
			<div className="hero col-md-3">
				<div className="heroWrapper">
					...
					<button type="button" className="addToTeam btn btn-primary" onClick={this.handleClick}>adicionar ao time</button>
				</div>
			</div>
		);
	}
		...
		return (
			<form className="searchForm row form-inline" onSubmit={this.handleSubmit}>
				<div className="col-md-6">
					...
					<input type="submit" value="Buscar" className="btn btn-primary" /><br />
				</div>
				<div className="col-md-6 right">
					<button type="button" className="btn" onClick={this.handleCancel}>limpar resultados</button>
				</div>
			</form>
		);
	}
...
var CurrentTeam = React.createClass({
	...
	render: function(){
		that = this
		const nodes = this.props.data.map((hero, index) => <CurrentTeamItem key={index} name={hero.name} image={hero.image} delete={that.delete}/>)
		return <div className="currentTeam"><h2>My Team</h2>{nodes}</div>
	}
});

const CurrentTeamItem = React.createClass({
	delete: function(){
		id = this.props.key
